function Build-EnterpriseAppMapSvg {
    <#
        SVG map of enterprise apps (service principals) as detail cards in a
        masonry layout, sourced from the dedicated EnterpriseApps export (every
        service principal, including gallery apps and managed identities). Each
        card shows the app name in bold, then every field the export carries (IDs,
        SP type and origin, whether an app registration exists, account state,
        owner, SSO configuration, assignment requirement, homepage, tags,
        credentials and their expiry, sign-in activity). Card fill signals SSO
        state (green = SSO configured, grey = not), and a red border flags a
        disabled account. Cards vary in height, so they are packed into the
        shortest column for a tight fit.
    #>
    [CmdletBinding()]
    param(
    ...
    $data = Get-EnterpriseAppMapData    -SourceFolder $SourceFolder
    $full = Get-EnterpriseAppFullData   -SourceFolder $SourceFolder
    if (-not $data -or -not $full) { Write-Host "No enterprise app data in this export." -ForegroundColor Yellow; return }
    $tenant = $data.Tenant
    ...
    $dsBlock = Get-MapDataSourceBlock -SourceFolder $SourceFolder -BaseName 'EnterpriseApps' -WidthIn $dsW -FontSize 10 -LineH $lineH -Pad $cardPad -GridFields
    $dsH = $dsBlock.Height

    # build each card's wrapped lines + height first. Long values wrap to the
    # card's usable width so nothing spills outside the block; the card grows to
    # fit the extra lines.
    $cards = @()
    $msHidden = 0
    $ssoShown = 0
    foreach ($app in $full.Apps) {
        # By default hide Microsoft first-party apps (as the portal does). The
        # IsMicrosoftApp flag is written by the enterprise collector; on older
        # exports without it, nothing is hidden.
        if (-not $IncludeMicrosoftApps -and "$($app.IsMicrosoftApp)".Trim() -eq 'Yes') { $msHidden++; continue }
        if ("$($app.SSOConfigured)".Trim().ToLower() -eq 'yes') { $ssoShown++ }
        $detail = @(Get-AppDetailLines -App $app -View 'enterprise')
        $ssoOn   = ("$($app.SSOConfigured)".Trim().ToLower() -eq 'yes')
        $enabled = ("$($app.AccountEnabled)".Trim().ToLower() -eq 'yes')
        $fill = if ($ssoOn) { '#C8E6C9' } else { '#E0E0E0' }        # green configured / grey not
        $edge = if ($enabled) { '#B8BFC7' } else { '#C81E1E' }      # red edge = account disabled
        ...
        $cards += [pscustomobject]@{
            Name = $app.AppName; Lines = $lines; Height = $h; Fill = $fill; Edge = $edge
        }
    }
    ...
    $tenantLines = if ($tenant) {
        @(
            ...
            @{ Text = ("Enterprise apps: {0}   (SSO configured: {1})" -f $cards.Count, $ssoShown); Bold = $false; Align = 'start' }
    ...
    $legendLines = @(
        ...
        @{ BoldPrefix = 'Red edge: '; Text = 'service principal disabled'; Align = 'start' }
    )
    ...
    Write-Host "  SVG: $($cards.Count) enterprise apps shown ($msHidden Microsoft apps hidden), detail cards (SSO configured: $ssoShown)" -ForegroundColor DarkGray
    return $svg
}
